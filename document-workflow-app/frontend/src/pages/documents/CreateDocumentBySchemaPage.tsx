...
export const CreateDocumentBySchemaPage = () => {
  ...
  useEffect(() => {
    if (versionQuery.data) {
      form.setFieldValue("document_type_version_id", versionQuery.data.id);
    }
  ...
  const onFinish = (values: Record<string, unknown>) => {
    ...
    for (const code of dynamicFieldCodes) {
      dataJson[code] = values[code];
    }
  ...
  return (
    <Card title="Создание документа">
      ...
      <Form form={form} layout="vertical" onFinish={onFinish}>
        <Space align="start" wrap style={{ width: "100%" }}>
          ...
          <Form.Item name="document_type_id" label="Тип документа" rules={[{ required: true }]}>
            <Select
              ...
              style={{ width: 320 }}
              options={(typesQuery.data ?? []).map((item) => ({ value: item.id, label: `${item.code} - ${item.name}` }))}
            />
          </Form.Item>
          <Form.Item name="document_type_version_id" label="Версия формы" rules={[{ required: true }]}>
            <Input style={{ width: 260 }} disabled />
          </Form.Item>
          <Form.Item name="number" label="Номер" rules={[{ required: true }]}>
            <Input style={{ width: 180 }} placeholder="PAY-000001" />
          </Form.Item>
          <Form.Item name="document_date" label="Дата документа" rules={[{ required: true }]} initialValue={dayjs()}>
        ...
        <Collapse
          style={{ marginTop: 16 }}
          items={[{ key: "debug", label: "Debug schema", children: <pre style={{ whiteSpace: "pre-wrap" }}>{JSON.stringify(schema ?? {}, null, 2)}</pre> }]}
        />
